...
const Navbar = () => {
  ...
  const { setShowsSearch, getCartCount, navigate, token, setToken, setCartItems } = useContext(ShopContext)
  ...
  return (
    <div className='flex item-center justify-between font-medium'>
      ...
      <div className='flex items-center gap-8'>
        <img onClick={() => setShowsSearch(true)} src={assets.search_icon} className='w-5 cursor-pointer' alt="" />
        ...
        <div className='group relative'>
          ...
          {token && 
            ...
            <div className='flex flex-col gap-2 w-36 py-5 bg-slate-100 text-gray-500 rounded'>
              ...
              <p onClick={()=>navigate('/orders')} className='cursor-pointer ml-3 hover:text-black'>Orders</p>
              <p onClick={logout} className='cursor-pointer ml-3 hover:text-black'>Logout</p>
            </div>
          </div>
          }
        </div>
      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'} `}>
        <div className='flex flex-col text-gray-600'>
          ...
          <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 font-serif border' to='/collection'>COLLECTION</NavLink>
          <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 font-serif border' to='/about'>ABOUT</NavLink>
          <NavLink onClick={() => setVisible(false)} className='py-2 pl-6 font-serif border' to='/contact'>CONTACT</NavLink>
        </div>
      </div>
    </div>
  )
